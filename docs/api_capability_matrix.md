...
| Parameter summary and discovery | `/parameter_summary` | `ableton_bridge.parameter_summary` | yes | yes | n/a | yes | yes | Preserves the whole-Set summary. Shared bounded reads are Live-validated. Parameter metadata now includes `automation_state` (`none`, `active`, `overridden`); this new diagnostic field awaits Hub reload validation. Display formatting and quantized labels remain opt-in. |
...
| Multi-parameter control | `/set_parameters` | `ableton_bridge.multi_parameter_control` | yes | yes | yes | yes | yes | Ordinary behavior is preserved. Dry-run and readback now include parameter automation state; this diagnostic field awaits reload validation. Return/Main stable-id behavior remains validated. |
