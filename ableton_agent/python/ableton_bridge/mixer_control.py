...
def parse_change(text: str) -> dict[str, Any]:
    ...
    if len(parts) == 3:
        ...
        return {"track": int(track) if track.isdigit() else track, "field": field, **(parse_ui_literal(value) or {"value": _typed_value(value)})}
    if len(parts) == 4:
        ...
        return {
            ...
            **(parse_ui_literal(value) or {"value": _typed_value(value)}),
        }
...
def set_mix(
    ...
    *,
    ...
    decision = resolve_execution("set_mix", execution=execution, commit=commit)
...
def restore_mix(operation_id: str, *, timeout: float = 3.0, journal_path=None, **connection: Any) -> dict[str, Any]:
    receipt = load_receipt(operation_id, path=journal_path)
    if receipt.get("route") != "/set_mix":
        raise MixerControlError(f"operation {operation_id} is not a mixer operation")
    return set_mix(receipt.get("restore_changes", []), execution="apply", timeout=timeout, **connection)


def main() -> int:
    parser = argparse.ArgumentParser(description="Inspect, apply, or restore track mixer state")
    parser.add_argument(
        "--change",
        action="append",
        required=False,
    ...
    try:
        if args.restore:
            result = restore_mix(args.restore, timeout=args.timeout, host=args.host, command_port=args.command_port, reply_port=args.reply_port)
            print(json.dumps({"ok": True, "result": result}, indent=2, ensure_ascii=False), flush=True)
            return 0
        if not args.change:
            raise MixerControlError("--change is required unless --restore is used")
    ...
    except (MixerControlError, ExecutionPolicyError, OperationJournalError, ValueDisplayError, json.JSONDecodeError, ValueError) as error:
