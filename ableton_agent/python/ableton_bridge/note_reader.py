...
class NoteReadError(RuntimeError):
    pass


class NoteReadTimeoutError(NoteReadError):
    pass
...
def read_notes(
    ...
    packet = encode_message("/notes", [request_id])
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as reply_socket:
        ...
        while True:
            ...
            if remaining <= 0:
                raise NoteReadTimeoutError(
                    f"No notes reply from Ableton Agent Note Reader on UDP {reply_port}; "
                    "load Ableton Agent Note Reader.amxd in the current Set"
                )
            ...
            try:
                ...
                continue
            if path not in ["/notes", "notes"] or len(arguments) < 2:
                continue
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Read MIDI notes from current detail clip")
    ...
    except (NoteReadError, json.JSONDecodeError) as error:
