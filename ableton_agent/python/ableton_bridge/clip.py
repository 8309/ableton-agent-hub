        ...
        while True:
            ...
            if remaining <= 0:
                raise ClipTimeoutError(
                    f"No clip reply from Ableton Agent Clip on UDP {reply_port}; "
                    "load Ableton Agent Clip.amxd in the current Set"
                )
            ...
            try:
                ...
                continue
            if path not in ["/clip", "clip"] or len(arguments) < 5:
                continue
            if arguments[0] != request_id:
                continue
            return {
                "has_clip": bool(int(arguments[1])),
                "is_midi": bool(int(arguments[2])),
                "name": str(arguments[3]),
                "length": float(arguments[4]),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Read current detail clip through Ableton Agent Clip.amxd")
