...
class TracksTimeoutError(TracksError):
    pass
...
def read_tracks(
    ...
    packet = encode_message("/tracks", [request_id])
    ...
    with reply_port_lock(reply_port, timeout=timeout), socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as reply_socket:
        ...
        while True:
            ...
            if remaining <= 0:
                raise TracksTimeoutError(
                    f"No tracks reply from Ableton Agent Tracks on UDP {reply_port}; "
                    "load Ableton Agent Tracks.amxd in the current Set"
                )
            ...
            try:
                ...
                continue
            if path not in ["/tracks", "tracks"] or len(arguments) < 2:
                continue
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Read tracks through Ableton Agent Tracks.amxd")
    ...
    except (TracksError, json.JSONDecodeError) as error:
