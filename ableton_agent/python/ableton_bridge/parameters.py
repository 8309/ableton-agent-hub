...
class ParametersTimeoutError(ParametersError):
    pass
...
def read_parameters(
    ...
    packet = encode_message("/parameters", [request_id])
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as reply_socket:
        ...
        while True:
            ...
            if remaining <= 0:
                raise ParametersTimeoutError(
                    f"No parameters reply from Ableton Agent Parameters on UDP {reply_port}; "
                    "load Ableton Agent Parameters.amxd in the current Set"
                )
            ...
            try:
                ...
                continue
            if path not in ["/parameters", "parameters"] or len(arguments) < 2:
                continue
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Read selected track device parameters")
    ...
    try:
        result = read_parameters(
            host=args.host,
            command_port=args.command_port,
            reply_port=args.reply_port,
            timeout=args.timeout,
        )
    except (ParametersError, json.JSONDecodeError) as error:
