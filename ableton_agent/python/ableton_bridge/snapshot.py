...
def read_snapshot(
    ...
    packet = encode_message("/snapshot", [request_id])
        ...
        while True:
            ...
            if remaining <= 0:
                raise SnapshotTimeoutError(
                    f"No snapshot reply from Ableton Agent Snapshot on UDP {reply_port}; "
                    "load Ableton Agent Snapshot.amxd in the current Set"
                )
            ...
            try:
                ...
                continue
            if path not in ["/snapshot", "snapshot"] or len(arguments) < 2:
                continue
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Read all tracks and device names through Ableton Agent Snapshot.amxd")
